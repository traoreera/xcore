...
## 2026-05-15 - [EventBus dispatch optimization with pre-compiled wildcards]
**Learning:** Iterating over all registered event patterns with `fnmatch.fnmatch` during dispatch is an $O(N)$ operation that becomes a bottleneck as the number of handlers grows. Exact match lookups should be $O(1)$, and wildcard lookups should only iterate over actual wildcard patterns.
**Action:** Separate exact and wildcard subscriptions. Use an exact dictionary lookup for dispatch and pre-compile wildcard patterns into regexes during the subscription phase. Additionally, store the source pattern in the handler entry to allow $O(1)$ cleanup of one-shot handlers. This yielded a ~3.7x speedup for exact matches and a ~47x speedup for no-match cases.
