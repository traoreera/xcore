...
async def _mkt_list(args) -> None:
    ...
    with console.status("[bold green]🔍 Récupération du catalogue...[/]"):
        try:
            ...
            err_console.print(f"[bold red]❌ Erreur marketplace :[/] {escape(str(e))}")
            sys.exit(1)
...
async def _mkt_trending(args) -> None:
    ...
    table.add_column("Version", style="magenta")
    table.add_column("Note", justify="center")
    table.add_column("Downloads", justify="right", style="bold")
    table.add_column("Description", style="white")
...
async def _mkt_show(args) -> None:
    ...
    if not plugin:
        err_console.print(f"[bold red]❌ Plugin '{escape(name)}' introuvable.[/]")
        sys.exit(1)
...
async def _mkt_rate(args) -> None:
    ...
    try:
        ...
        err_console.print(f"[bold red]❌ Erreur :[/] {escape(str(e))}")
        sys.exit(1)
...
def _stars(rating: float) -> str:
    """Convertit une note 0-5 en étoiles avec markup Rich."""
    try:
