...
class DatabaseServiceProvider(BaseServiceProvider):
    async def init(self, container: ServiceContainer) -> None:
        ...
        for name, adapter in mgr.adapters.items():
            container.register_service(name, adapter)
        if mgr.adapters:
            first = next(iter(mgr.adapters.values()))
            if not container.has("db"):
                container.register_service("db", first)
...
class SchedulerServiceProvider(BaseServiceProvider):
    async def init(self, container: ServiceContainer) -> None:
        ...
        container.register_service("scheduler", svc)
...
class ExtensionServiceProvider(BaseServiceProvider):
    async def init(self, container: ServiceContainer) -> None:
        ...
        for name, ext in loader.extensions.items():
            container.register_service(f"ext.{name}", ext)
...
class ServiceContainer:
    ...
    def register_service(self, name: str, service: Any) -> None:
        ...
        # Priorité au registre pour le scoping et la protection
        if self._registry:
            try:
                self._registry.register_core_service(name, service)
            except Exception as e:
                logger.warning(f"Erreur enregistrement registre pour '{name}': {e}")
                # En cas de protection par le registre, on n'enregistre pas localement
    ...
    def get(self, name: str) -> T:
        ...
        """
        # Priorité au registry pour le respect des scopes
        if self._registry:
            try:
                return self._registry.get_service(name, requester="kernel")
