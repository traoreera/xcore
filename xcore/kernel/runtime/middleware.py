...
class MiddlewarePipeline:
    ...
    async def execute(self, plugin_name: str, action: str, payload: dict, **kwargs) -> dict:
        """Démarre l'exécution de la chaîne."""

        async def _chain(index: int, p_name: str, act: str, pay: dict, **kw) -> dict:
            if index < len(self._middlewares):
                ...
                return await middleware(
                    ...
                    act,
                    pay,
                    lambda pn, a, pl, **k: _chain(index + 1, pn, a, pl, **k),
                    **kw
                )
            return await self._final_handler(p_name, act, pay, **kw)

        return await _chain(0, plugin_name, action, payload, **kwargs)
